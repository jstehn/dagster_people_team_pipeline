...
from dlt.sources.helpers.rest_client import RESTAPIConfig, rest_api_source
from dlt.sources.helpers.rest_client.auth import BasicAuth
...
@dlt.source(name="bamboohr")
def bamboohr_source(
    api_key: str = dlt.secrets.value,
    subdomain: str = dlt.secrets.value,
    report_id: str = dlt.secrets.value
) -> Any:
    # Create a REST API configuration for the BambooHR API
    # Use RESTAPIConfig to get autocompletion and type checking
    config: RESTAPIConfig = {
        "client": {
            "base_url": f"https://api.bamboohr.com/api/gateway.php/{subdomain}/v1/",
            # we add an auth config if the API key is present
        ...
        "resources": {
            ...
            "report": {
                "endpoint": "reports/{report_id}?format=json&onlyCurrent=true",
                "pagination": {
                    "type": "none",
                },
            },
    ...
    # Check the connection to the BambooHR API
    check_connection(config)

    # Create and return the REST API source
